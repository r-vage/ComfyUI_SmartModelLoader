...
def apply_lora_to_svdq_linear(
    linear: SVDQW4A4Linear, lora_down: torch.Tensor, lora_up: torch.Tensor,
):
...
def concat_lora_weights(
    ...
    """
    assert (
        len(new_downs) == len(new_ups) == len(strengths)
    ), "Lengths of new_downs, new_ups, and strengths must match."
    assert (base_down is None) == (
        base_up is None
    ), "Both base_down and base_up should be None or not None."

    combined_new_downs = torch.cat(
        [nd * s for nd, s in zip(new_downs, strengths, strict=False)], dim=0,
    )
    ...
    assert (
        base_up.shape[1] == base_down.shape[0]
    ), "Base up and down weights shapes do not match."
...
class ZImageModelPatcher(ModelPatcher):
    ...
    def unpatch_model(self, device_to=None, unpatch_weights=True):
        ...
        if unpatch_weights:
            for k, (proj_down, proj_up) in self.svdq_backup.items():
                logging.debug(
                    f"<<<< Unpatching svdq_linears, key: {k}, device_to: {device_to}",
                )
                attr_name = k.replace(".qweight", "")
                linear = get_attr(self.model, attr_name)
                assert isinstance(linear, SVDQW4A4Linear)
                if device_to is not None:
                    proj_down, proj_up = proj_down.to(device=device_to), proj_up.to(
                        device=device_to,
                    )
    ...
    def do_svdq_linear_backup(self, key: str, linear: SVDQW4A4Linear):
        assert key.endswith(
            ".qweight",
        ), f">>> Unexpected key {key} to do svdq_linear backup"
    ...
    def partially_unload(self, device_to, memory_to_free=0, force_patch_weights=False):
        ...
        with self.use_ejected():
            ...
            for unload in unload_list:
                ...
                if hasattr(m, "comfy_patched_weights") and m.comfy_patched_weights:
                    ...
                    if move_weight:
                        ...
                        logging.debug(f"freed {n}")

                        for param in params:
                            self.pin_weight_to_device(f"{n}.{param}")
            ...
            logging.info(
                f"Unloaded partially: {memory_freed / (1024 * 1024):.2f} MB freed, {self.model.model_loaded_weight_memory / (1024 * 1024):.2f} MB remains loaded, {offload_buffer / (1024 * 1024):.2f} MB buffer reserved, lowvram patches: {self.model.lowvram_patch_counter}",
            )
            return memory_freed
    ...
    def patch_weight_to_device(self, key, device_to=None, inplace_update=False):
        ...
        assert key.endswith(".qweight") or key.endswith(
            ".weight",
        ...
        if isinstance(linear, nn.Linear):
            ...
            if ".qkv." in key:
                patches_list_for_key = self.patches.get(
                    key.replace(".qweight", ".weight"), [],
                )
                logging.debug(
                ...
                if len(patches_list_for_key) > 0:
                    ...
                    for (
                        ...
                        if offset[1] == 0:
                            ...
                            raise ValueError(
                                f"Invalid offset {offset} for qkv LoRA patch in key {key}",
                            )

                    assert len(q_patches) == len(k_patches) == len(v_patches)
                    composed_up, composed_down = None, None
                    for q_patch, k_patch, v_patch in zip(
                        q_patches, k_patches, v_patches, strict=False,
                    ):
                        q_strength, q_up, q_down, q_alpha, q_rank = _parse_patch(
                            q_patch,
                        )
                        k_strength, k_up, k_down, k_alpha, k_rank = _parse_patch(
                            k_patch,
                        )
                        v_strength, v_up, v_down, v_alpha, v_rank = _parse_patch(
                            v_patch,
                        )
                        composed_down, composed_up = concat_lora_weights(
                            ...
                            [
                                (
                                    q_strength * (q_alpha / q_rank)
                                    if q_alpha is not None
                                    else q_strength
                                ),
                                (
                                    k_strength * (k_alpha / k_rank)
                                    if k_alpha is not None
                                    else k_strength
                                ),
                                (
                                    v_strength * (v_alpha / v_rank)
                ...
                w1_patches = self.patches.get(
                    key.replace("w13.qweight", "w1.weight"), [],
                )
                w3_patches = self.patches.get(
                    key.replace("w13.qweight", "w3.weight"), [],
                )
                ...
                if len(w1_patches) > 0:
                    composed_up, composed_down = None, None
                    for w1_patch, w3_patch in zip(w1_patches, w3_patches, strict=False):
                        w1_strength, w1_up, w1_down, w1_alpha, w1_rank = _parse_patch(
                            w1_patch,
                        )
                        w3_strength, w3_up, w3_down, w3_alpha, w3_rank = _parse_patch(
                            w3_patch,
                        )
                        composed_down, composed_up = concat_lora_weights(
                            composed_down,
                            ...
                            [
                                (
                                    w3_strength * (w3_alpha / w3_rank)
                                    if w3_alpha is not None
                                    else w3_strength
                                ),
                                (
                                    w1_strength * (w1_alpha / w1_rank)
            ...
            raise ValueError(
                f"Unsupported module type for LoRA patching: {type(linear)}, key: {key}",
            )
